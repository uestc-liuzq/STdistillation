...
def evaluate_synset(net, x_sys,y_sys, test_loader, args):
    ...
    for ep in range(Epoch + 1):
        ...
        if ep == Epoch:
            with torch.no_grad():
                mae, mse, rmse = model_test(args, net, optimizer, criterion, test_loader)
    ...
    print('%s Evaluate: epoch = %04d train time = %.4f s train loss = %.6f , test mae = %.4f, '
        ...
        get_time(), Epoch, time_train, loss_train,mae, mse, rmse))

    return net, acc_train_list, mae, mse, rmse
