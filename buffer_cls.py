...
from process.exp_cls import model_train, model_test
from network_patch import ST_Model
import warnings
...
def main(args):
    ...
    for it in range(0, args.num_experts):
        ...
        ''' Train synthetic '''
        teacher_net = ST_Model(args)
        teacher_net = teacher_net.cuda()
        teacher_net.train()
        lr = args.lr_teacher
        teacher_optim = torch.optim.Adam(teacher_net.parameters(), lr=args.learning_rate)  # optimizer_img for synthetic ECG200
        ...
        for e in range(train_epochs):
            train_loss, model = model_train(args, teacher_net, teacher_optim, criterion, train_loader,e)
            accuracy = model_test(args, model,test_loader)

            print("Itr: {}\tEpoch: {}\tTrain Loss: {}\nTest Accuracy: {}\n".format(it, e,train_loss,accuracy))
...
if __name__ == '__main__':
    ...
    parser.add_argument('--data_name', type=str, default='ECG200')
    ...
    parser.add_argument('--dsid', type=str, default='ECG200', help='Classification Dataset ID')
    parser.add_argument('--labels', type=int, required=True, default=2, help='TS labels')
